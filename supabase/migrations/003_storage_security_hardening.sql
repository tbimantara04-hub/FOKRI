-- FOKRI GAMES XII
-- Remote-safe Storage security hardening.
-- Prerequisites:
--   1. Supabase Storage is enabled and storage.objects exists.
--   2. public.profiles, public.registrations, public.competition_admins,
--      public.verifier_assignments, and public.is_super_admin() already exist.
--   3. The authenticated role exists.
--   4. The private schema is not included in Supabase's exposed schemas.
-- This migration does not create users, modify auth.users, or alter application tables.
-- The private schema must not be included in Supabase's exposed schemas.
...
as $$
begin
  if not public.is_valid_document_storage_path(path) then
    return null;
  end if;
...
$$;
  ...
  -- PostgreSQL grants EXECUTE to PUBLIC by default. Storage RLS evaluates these
  -- functions as the authenticated role, so retain only that role's execution
  -- privilege. This blocks anon/public execution while preserving RLS behavior.
  revoke execute on function private.is_super_admin() from public;
  grant execute on function private.is_super_admin() to authenticated;

  revoke execute on function public.is_valid_document_storage_path(text) from public;
  grant execute on function public.is_valid_document_storage_path(text) to authenticated;

  revoke execute on function public.document_storage_user_id(text) from public;
  grant execute on function public.document_storage_user_id(text) to authenticated;

  revoke execute on function public.document_storage_competition_id(text) from public;
  grant execute on function public.document_storage_competition_id(text) to authenticated;

  revoke execute on function public.document_storage_registration_id(text) from public;
  grant execute on function public.document_storage_registration_id(text) to authenticated;
